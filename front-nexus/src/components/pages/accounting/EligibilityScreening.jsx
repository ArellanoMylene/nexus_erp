    ...
    <div className=" p-3 sm:p-4 transition-colors duration-500">
      <div className="w-full max-w-7xl mx-auto space-y-4 font-sans">
        <div className="flex justify-between items-center border-b border-slate-200  pb-3">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900  flex items-center gap-2"><CheckSquare size={24} className="text-indigo-600" />Eligibility Screening</h2>
          <span className="text-sm text-slate-500  font-medium">Application Screening</span>
        </div>

        {statistics && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-white  rounded-md border border-slate-200  shadow-sm p-4"><p className="text-xs font-medium text-slate-600 ">Total Screenings</p><p className="text-2xl font-bold text-indigo-600  mt-1">{statistics.total_screenings}</p></div>
            <div className="bg-white  rounded-md border border-slate-200  shadow-sm p-4"><p className="text-xs font-medium text-slate-600 ">Passed</p><p className="text-2xl font-bold text-green-600  mt-1">{statistics.passed_count}</p></div>
            <div className="bg-white  rounded-md border border-slate-200  shadow-sm p-4"><p className="text-xs font-medium text-slate-600 ">Failed</p><p className="text-2xl font-bold text-red-600  mt-1">{statistics.failed_count}</p></div>
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="relative flex-grow max-w-xs">
              <input type="text" placeholder="Search screenings..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-8 pr-3 py-2 rounded-md border border-slate-300  focus:outline-none focus:ring-2 focus:ring-indigo-500   text-sm transition-all shadow-inner" />
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors text-sm font-medium border border-indigo-700  shadow-md shadow-indigo-500/30 whitespace-nowrap"><Plus size={14} />New Screening</button>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 ">
              ...
              <tbody className="divide-y divide-slate-100  bg-white ">
                {currentItems.length > 0 ? currentItems.map((scr) => (
                  <tr key={scr.screening_id} className="text-sm text-slate-700  hover:bg-indigo-50/50  transition duration-150">
                    <td className="px-4 py-2"><span className="font-mono text-xs bg-slate-100  px-2 py-0.5 rounded">{scr.application_number}</span></td>
                    <td className="px-4 py-2"><div className="font-medium">{scr.student_name}</div><div className="text-xs text-slate-500 ">{scr.student_number}</div></td>
                    <td className="px-4 py-2"><div className="font-semibold text-slate-900 ">{scr.scholarship_name}</div></td>
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => handleEdit(scr)} className="text-blue-600 hover:text-blue-800   transition-colors p-1 rounded-full hover:bg-slate-200 " title="Edit"><Edit size={14} /></button>
                        <button onClick={() => handleDelete(scr.screening_id)} className="text-red-600 hover:text-red-800   transition-colors p-1 rounded-full hover:bg-slate-200 " title="Delete"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                )) : (<tr><td colSpan="9" className="p-4 text-center text-slate-500  italic">No screenings found.</td></tr>)}
              </tbody>
            </table>
          </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-2 z-50" onClick={closeModal}>
          <div className="bg-white  rounded-lg shadow-2xl w-full max-w-4xl border border-slate-200  max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 flex justify-between items-center px-4 py-3 border-b border-slate-200  bg-slate-50  rounded-t-lg z-10">
              <h3 className="text-lg font-bold text-slate-900 ">{currentScreening ? "Edit Screening" : "New Screening"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-3">
              <div>
                <label className="block text-xs font-medium text-slate-700  mb-1">Application *</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700  mb-1">Screening Date *</label>
                <input type="date" name="screening_date" value={formData.screening_date} onChange={handleInputChange} required className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
              </div>
              <div className="border border-slate-200  rounded-md p-3">
                <h4 className="text-sm font-semibold text-slate-900  mb-2">Eligibility Criteria</h4>
                <div className="grid grid-cols-2 gap-3">
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">GPA Requirement Met</span>
                  </label>
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">Year Level Eligible</span>
                  </label>
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">Course Eligible</span>
                  </label>
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">Income Requirement Met</span>
                  </label>
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">Documents Complete</span>
                  </label>
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">Overall Eligible</span>
                  </label>
                </div>
              </div>
              <div className="border border-slate-200  rounded-md p-3">
                <h4 className="text-sm font-semibold text-slate-900  mb-2">Interview Details</h4>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">Interview Required</span>
                  </label>
                  <label className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-700 ">Interview Completed</span>
                  </label>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-slate-700  mb-1">Interview Date</label>
                    <input type="date" name="interview_date" value={formData.interview_date} onChange={handleInputChange} className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-700  mb-1">Interview Score</label>
                    <input type="number" name="interview_score" value={formData.interview_score} onChange={handleInputChange} step="0.01" max="100" className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
                  </div>
                </div>
                <div className="mt-3">
                  <label className="block text-xs font-medium text-slate-700  mb-1">Interview Notes</label>
                  <textarea name="interview_notes" value={formData.interview_notes} onChange={handleInputChange} rows="2" className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors resize-vertical" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-700  mb-1">Eligibility Score</label>
                  <input type="number" name="eligibility_score" value={formData.eligibility_score} onChange={handleInputChange} step="0.01" max="100" className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-700  mb-1">Screening Status *</label>
                  <select name="screening_status" value={formData.screening_status} onChange={handleInputChange} required className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
                    <option value="Passed">Passed</option>
                    <option value="Failed">Failed</option>
                    <option value="Pending Review">Pending Review</option>
                    <option value="Conditional">Conditional</option>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700  mb-1">Disqualification Reasons</label>
                <textarea name="disqualification_reasons" value={formData.disqualification_reasons} onChange={handleInputChange} rows="2" className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors resize-vertical" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-700  mb-1">Recommendations</label>
                <textarea name="recommendations" value={formData.recommendations} onChange={handleInputChange} rows="2" className="w-full px-3 py-2 text-sm border border-slate-300  rounded-md   focus:ring-indigo-500 focus:border-indigo-500 transition-colors resize-vertical" />
              </div>
