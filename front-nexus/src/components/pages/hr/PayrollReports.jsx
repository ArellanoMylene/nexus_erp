      ...
      <div className="w-full max-w-7xl mx-auto space-y-4 font-sans">
        ...
        <div className="bg-white  p-4 rounded-lg shadow border border-slate-200  space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="flex items-end gap-2">
              <button
                ...
                disabled={loading}
                className="flex items-center gap-2 px-3 py-2 rounded-md font-medium transition-colors text-sm border shadow-sm whitespace-nowrap bg-indigo-600 text-white hover:bg-indigo-700 border-indigo-700  shadow-md shadow-indigo-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <FileText size={14} />
              ...
              {reportData && (
                <>
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded-md font-medium transition-colors text-sm border shadow-sm whitespace-nowrap bg-green-600 text-white hover:bg-green-700 border-green-700  shadow-md shadow-green-500/30"
                  >
                    <Download size={14} />
                    CSV
                  </button>
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded-md font-medium transition-colors text-sm border shadow-sm whitespace-nowrap bg-red-600 text-white hover:bg-red-700 border-red-700  shadow-md shadow-red-500/30"
                  >
                    <Download size={14} />
                    PDF
                  </button>
          ...
          <div className="bg-slate-50  p-3 rounded-lg border border-slate-200 ">
            <h4 className="font-semibold text-sm text-slate-900  mb-2">Report Types:</h4>
        ...
        {reportData && (
          <div className="bg-white  p-4 rounded-lg shadow border border-slate-200  space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="bg-white  border border-slate-200  rounded-lg p-4 border-l-4 border-l-indigo-600 hover:shadow-md transition-shadow">
                <p className="text-xs font-medium text-slate-600  uppercase">Total Employees</p>
                <p className="text-2xl font-bold text-indigo-600  mt-1">
                  {reportData.employee_count || 0}
                </p>
              </div>
              <div className="bg-white  border border-slate-200  rounded-lg p-4 border-l-4 border-l-green-600 hover:shadow-md transition-shadow">
                <p className="text-xs font-medium text-slate-600  uppercase">Total Gross Pay</p>
                <p className="text-2xl font-bold text-green-600  mt-1">
                  ₱{parseFloat(reportData.total_gross_pay || 0).toLocaleString()}
                </p>
              </div>
              <div className="bg-white  border border-slate-200  rounded-lg p-4 border-l-4 border-l-red-600 hover:shadow-md transition-shadow">
                <p className="text-xs font-medium text-slate-600  uppercase">Total Deductions</p>
                <p className="text-2xl font-bold text-red-600  mt-1">
                  ₱{parseFloat(reportData.total_deductions || 0).toLocaleString()}
                </p>
              </div>
              <div className="bg-white  border border-slate-200  rounded-lg p-4 border-l-4 border-l-blue-600 hover:shadow-md transition-shadow">
                <p className="text-xs font-medium text-slate-600  uppercase">Total Net Pay</p>
                ...
                <table className="min-w-full divide-y divide-slate-200 ">
                  ...
                  <tbody className="divide-y divide-slate-100  bg-white ">
                    {reportData.payslips.map((payslip, index) => (
                      <tr key={index} className="text-sm text-slate-700  hover:bg-indigo-50/50  transition duration-150">
                        ...
                        <td className="px-4 py-2 text-right font-semibold text-green-600 ">₱{parseFloat(payslip.gross_pay || 0).toLocaleString()}</td>
                        <td className="px-4 py-2 text-right font-semibold text-red-600 ">₱{parseFloat(payslip.total_deductions || 0).toLocaleString()}</td>
                        <td className="px-4 py-2 text-right font-bold text-indigo-600 ">₱{parseFloat(payslip.net_pay || 0).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {reportType === "Department" && reportData.payslips && reportData.payslips.length > 0 && (
              <div className="space-y-6">
                {Object.entries(
                  ...
                  return (
                    <div key={dept} className="rounded border border-slate-200  overflow-hidden">
                      ...
                      <table className="min-w-full divide-y divide-slate-200 ">
                        ...
                        <tbody className="divide-y divide-slate-100  bg-white ">
                          ...
                          <tr className="bg-slate-50  font-semibold text-sm">
                            ...
                            <td className="px-4 py-2 text-right text-green-600 ">₱{deptTotals.gross.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            <td className="px-4 py-2 text-right text-red-600 ">₱{deptTotals.deductions.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            <td className="px-4 py-2 text-right text-indigo-600 ">₱{deptTotals.net.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
            ...
            {reportType === "Bank Transfer" && reportData.payslips && reportData.payslips.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-semibold text-sm text-slate-900  border-b border-slate-200  pb-2">
